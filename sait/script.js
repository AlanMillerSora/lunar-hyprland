...
const DESKTOP_NAMES = ['I','II','III','IV','V','VI','VII','VIII'];
const ECLIPSE_COVERAGE = [0.08, 0.40, 0.75, 1.00, 0.68, 0.35, 0.12, 0];

let currentDesktop = 4;
...
if (URL_PHASE >= 1 && URL_PHASE <= 8) currentDesktop = URL_PHASE;
...
function setEclipsePhase(num) {
  ...
  desktop.classList.remove(
    ...
    'eclipse-phase-5', 'eclipse-phase-6', 'eclipse-phase-7', 'eclipse-phase-8'
  );
...
const EX_OFF = [0, -319, -207, -107, 0, 107, 207, 274, 320];
...
function exportPhaseSVG(n) {
  ...
  const horizonEl = P.horizon === 'blood'
    ? `<ellipse cx="${EX_CX}" cy="1150" rx="1500" ry="360" fill="#d22837" opacity="0.5" filter="url(#blurHorizon)"/>` +
      `<ellipse cx="${EX_CX}" cy="1092" rx="1400" ry="10" fill="#ffffff" opacity="0.4" filter="url(#blurHorizon)"/>`
    : P.horizon
  ...
  const redGlow = P.blood
    ? `<circle cx="${mx}" cy="${EX_CY}" r="215" fill="url(#redGlow)" opacity="0.55"/>`
    : '';
